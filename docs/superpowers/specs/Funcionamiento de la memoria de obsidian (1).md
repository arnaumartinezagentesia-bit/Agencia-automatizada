...
obsidian\_vault/  
├── 00\_raw\_inputs/            \# Buzón de entrada: Papers, PDFs, recortes web y wikis externas  
├── 00\_core\_shared/           \# Conocimiento transversal y VAULT\_RULES.md  
├── 01\_office\_trading/        \# Notas procesadas del departamento  
...

**Nuevo Protocolo: Ingesta de Fuentes Externas**

1. **Entrada de Archivos:** Guardas cualquier documento externo (paper, recorte de *Obsidian Web Clipper*, PDF) en `./obsidian_vault/00_raw_inputs/`.  
2. **Procesamiento del Curador:** Durante el mantenimiento (o por comando manual), el agente lee el documento de `00_raw_inputs/`, extrae las lecciones principales, crea o actualiza la nota `.md` en la oficina correspondiente (ej. `01_office_trading/`) y actualiza el `index.md`.  
3. **Limpieza:** Una vez sintetizada la información en la Wiki, el archivo original se traslada a `./obsidian_vault/00_raw_inputs/archive/`.

Con este pequeño ajuste añadido al archivo de especificación, Claude Code sabrá que debe programar dos rutas de aprendizaje: una para la experiencia de los agentes y otra para el conocimiento externo que tú le proporciones.